...
-- Broader correction for current launch imports:
-- Some existing public-domain rows were created with Czech as a UI/default value
-- even though their actual edition text and metadata are English. When Czech
-- localization is empty and English/legacy title exists, normalize the edition
-- language to English. This is still data-based, not UI-locale-based.
update works
set
  canonical_language = 'en',
  edition_language = 'en',
  original_language = case
    when original_language is null or trim(coalesce(original_language, '')) = '' or lower(original_language) = 'cs'
      then 'en'
    else original_language
  end
where
  origin_type = 'public_domain'
  and lower(coalesce(canonical_language, edition_language, '')) = 'cs'
  and title_cs is null
  and (
    title_en is not null
    or title is not null
  );

-- Product placeholder cleanup intentionally removed in v0.10.9f.
-- The current database does not have a work_products table in this environment.
-- Placeholder copy is handled safely in the UI layer instead.
