...
import { getTagBySlugForEdit, getTagsForMember } from "@/lib/dbTags";
import { mapTagToFormValues } from "@/lib/forms/tagForm";
...
function getSuccessMessage(success?: string) {
  switch (success) {
    case "tag_created": return "Tag byl vytvořen.";
    case "tag_updated": return "Tag byl uložen.";
    default: return null;
  }
}
...
export default async function EditTagPage({ params, searchParams }: PageProps) {
  ...
  const tag = await getTagBySlugForEdit(slug);
  if (!tag) notFound();
  const defaults = mapTagToFormValues(tag);
  const updateTagWithSlug = updateTag.bind(null, slug);
  const allTags = (await getTagsForMember()).filter((item) => item.id !== tag.id);
  ...
  return (
    <main style={{ padding: "48px 32px", fontFamily: "serif", lineHeight: 1.6, maxWidth: "900px", margin: "0 auto" }}>
      ...
      <h1 style={{ fontSize: "40px", lineHeight: 1.1, marginTop: 0, marginBottom: "20px" }}>Editace tagu</h1>
      ...
      <form action={updateTagWithSlug} style={{ display: "grid", gap: "22px" }}>
        <section style={{ border: "1px solid #ddd", padding: "24px", display: "grid", gap: "18px" }}>
          ...
          <div>
            ...
            <select id="canonical_tag_id" name="canonical_tag_id" defaultValue={defaults.canonical_tag_id} style={{ width: "100%", padding: "12px 14px", border: "1px solid #ccc", fontSize: "16px" }}>
              ...
              {allTags.map((item) => <option key={item.id} value={item.id}>{item.label_cs}</option>)}
            </select>
          </div>
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
          <button type="submit" style={{ padding: "12px 18px", border: "1px solid #111", background: "#111", color: "#fff", cursor: "pointer", fontSize: "16px", fontWeight: 600 }}>Uložit změny</button>
          <Link href="/member/tags" style={{ padding: "12px 18px", border: "1px solid #ccc", textDecoration: "none", color: "#111" }}>Zpět</Link>
        </div>
      </form>
    </main>
  )
