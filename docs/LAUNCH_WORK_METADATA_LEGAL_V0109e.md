...
The product-card placeholder filter is now broader. It catches not only exact
`Coming soon`, but also values such as `Coming soon — PDF`, `coming-soon`,
`available soon`, and placeholder descriptions.

The SQL cleanup also nulls literal placeholder product titles/descriptions so
localized product type labels can be used.


## v0.10.9f follow-up

The SQL product cleanup block from v0.10.9e was removed because this database
environment does not have `work_products`. Placeholder product copy is still
handled in `WorkDetailClient.tsx`, so the UI fix remains active without requiring
a product-table data migration.
