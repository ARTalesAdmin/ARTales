...
Version 0.2 keeps the structured SVG source and adds a deterministic, self-contained rendering path for an artifact-only PNG. The PNG is **not committed** because binary files are not supported in this Codex PR flow. Instead, the manual GitHub Actions workflow installs `sharp@0.33.5` with `npm install --no-save` for artifact rendering only and generates the PNG as a downloadable artifact; until an explicit later decision says otherwise, the committed sources are the SVG, JSON metadata, registry, docs, generator, and workflow. The SVG embeds read-only copies of the referenced asset bytes for portable rendering; it does not change the authoritative source assets. The adjacent JSON remains the metadata and SHA-256 provenance source.
...
- committing generated PNG/JPG/WEBP/ICO/binary preview files;
